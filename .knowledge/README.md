...
- **VM Azure = homolog canonica** — desde 2026-07-29 (issue 997) consolidada na `hiram-demo-vm` (RG `HIRAM-DEMO-RG`, eastus2, IP estatico `20.98.234.200` via sslip.io), host COMPARTILHADO com as stacks jornada + levante. A stack (web+admin+api+worker+postgres+redis+storefront) roda de `/home/azureuser/easystok` SEM caddy proprio (`docker-compose.azure.noedge.yml`): o edge e o Caddy central em `/opt/hiram/deploy/stack` (Caddyfile de la roteia todos os vhosts). Deploy via cron `*/5` `scripts/docker/vm-deploy.sh` em modo pull + `EASYSTOK_COMPOSE_EXTRA`. Hosts: `app.`/`admin.`/`api.`/`casa.`/`casadababa.` `20.98.234.200.sslip.io`. A antiga `easystok-vm` (westus2, `20.230.185.203`) foi descomissionada (snapshot do disco preservado ~30 dias).
...
- **APK Casa da Baba**: OTA via `capacitor.config.json` `updateUrl` -> `api.20.98.234.200.sslip.io/api/mobile/apk/manifest`. Pareamento/sync do APK aponta pra VM via `build-release-wsl.sh` `CDB_API_BASE_URL`. Convencao: `apiBaseUrl` SEM `/api` (sync.js concatena `/api/mobile`). Gotcha: URL e IP via sslip.io — se o IP da VM mudar, OTA quebra (registrar dominio resolve). APKs buildados antes de 2026-07-29 apontam pro IP antigo (20.230.185.203) e NAO recebem OTA: precisam reinstalar.
