...
public class AnalyticsCalculationTests
{
    ...
    [Theory]
    [InlineData(100.00, 170.00, 70.00, 70.00)]   // caso base
    [InlineData(200.00, 280.00, 80.00, 40.00)]   // margem 40 %
    [InlineData(50.00,  50.00,  0.00,  0.00)]    // sem margem
    public void MargemPorProduto_CalculoDeveEstarCorreto(
        decimal custoMedio, decimal precoMedio, decimal margemAbs, decimal margemPct)
    {
 ...
 var pctCalculada = custoMedio > 0 ? Math.Round(margemCalculada / custoMedio * 100m, 2) : 0m;
    ...
    {
        decimal custo = 0m;   // var — não const, evita CS0020
        decimal preco = 150m;
    ...
    var margens = new List<MargemPorProduto>
 {
  new(Guid.NewGuid(), "B", 200m, 280m, 80m,  40m, 8),
       ...
       new(Guid.NewGuid(), "C", 300m, 330m, 30m,  10m, 5)
        ...
        ordenados[0].NomeProduto.Should().Be("A"); // 70 %
ordenados[1].NomeProduto.Should().Be("B"); // 40 %
        ordenados[2].NomeProduto.Should().Be("C"); // 10 %
 }
    ...
    [Theory]
    [InlineData(12000.00, 24, 500.00)]   // R$ 500 por venda
    [InlineData(7500.00,  15, 500.00)]
    [InlineData(0.00,      0,   0.00)]   // sem vendas
    public void ReceitaPorPeriodo_TicketMedioDeveEstarCorreto(
        decimal receitaBruta, int totalVendas, decimal ticketEsperado)
    {
...
.ToList();
        ...
        // Receita de dezembro (12 000) deve ser maior que janeiro (1 000)
    ...
    public void SazonalidadeMensal_MediaMovelTresMeses_PrimeiroMesDeveSerIgualAoProprioMes()
 {
  ...
  var dados = new[] { 30, 40, 50, 20, 60 };
      var medias = CalcularMediaMovel(dados);
        ...
        // índice 2: (30+40+50)/3 = 40
        medias[2].Should().Be(40m);
      // índice 3: (40+50+20)/3 = 36,67
  ...
  [Fact]
    ...
    [Theory]
    [InlineData(100, 2.0,  50)]   // 100 / 2 = 50 dias
    [InlineData(30,  0.5,  60)]   // 30 / 0,5 = 60 dias
    [InlineData(7,   1.0,   7)]
 public void ProjecaoRuptura_DiasAteRupturaDeveEstarCorreto(
        int quantidade, double taxa, int diasEsperados)
    {
  ...
  ? (int)Math.Floor(quantidade / taxaDecimal)
    ...
    {
        decimal taxa = 0m;    // var — não const, evita CS0020
  decimal estoque = 50m;
 ...
 new(Guid.NewGuid(), Guid.NewGuid(), "B", null, 50, 1m,   50, DateTime.UtcNow.AddDays(50)),
  ...
  ordenados[0].NomeProduto.Should().Be("A");  // mais crítico
        ordenados[1].NomeProduto.Should().Be("B");
        ordenados[2].NomeProduto.Should().Be("C");  // sem taxa ? sem ruptura prevista
    }
 ...
 // ??????????????????????????????????????????????????????????????????????????
    ...
    [Theory]
    [InlineData(2,  5, 1.4, 30, 40)]  // cobertura = ceil(1.4*30)=42 ? repor 42-2=40... ver regra
    [InlineData(0,  5, 0.0,  0, 10)]  // sem taxa ? 2 × mínimo ? 0
    public void ReposicaoSugerida_QuantidadeReposicaoDeveCobrirTrintaDias(
        int qtdAtual, int qtdMinima, double taxa, int diasCobertura, int qtdRepoEsperada)
    {
        _ = diasCobertura;
        // fórmula do AnalyticsRepository
    ...
    {
     // produto já vencido ontem ? dias = 0 (max com zero)
    ...
    [InlineData(0,  100.00,   0.00)]  // sem estoque ? sem risco
    public void ValidadeAlerta_ValorEmRiscoDeveSerQtdVezesCusto(
        int quantidade, decimal custo, decimal esperado)
    {
    ...
    {
decimal totalReceita = 0m;   // var — não const, evita CS0020
   decimal receitaCanal = 0m;
    ...
    [Theory]
    [InlineData(100, 30, 3.33)] // 100 unidades / 30 dias
 [InlineData(0,   30, 0.00)]   // sem saídas
    [InlineData(30, 1, 30.00)]  // 1 dia de janela
    public void Dashboard_MediaVendasDiariaDeveEstarCorreta(
        int totalSaidas, int dias, decimal mediaEsperada)
    {
   var diasEfetivos = Math.Max(1, dias);
    ...
    /// <summary>
    /// Réplica da lógica de média móvel de 3 meses do <see cref="AnalyticsRepository"/>.
    /// </summary>
