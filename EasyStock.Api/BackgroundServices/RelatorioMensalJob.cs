...
/// Executa no primeiro dia de cada mês para enviar resumos por email.
/// </summary>
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var nextRun = new DateTime(now.Year, now.Month, 1).AddMonths(1); // Primeiro dia do próximo mês
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            await emailService.SendAsync(
                empresa.Email!,
                $"Relatório Mensal - {mesAnterior:MMMM yyyy}",
                $"Segue o relatório mensal da sua empresa.<br><br><a href='{fileUrl}'>Download do Relatório</a>",
                isHtml: true);

            logger.LogInformation("Relatório mensal enviado para empresa {EmpresaId}", empresa.Id);
        }
        catch (Exception ex)
        {
