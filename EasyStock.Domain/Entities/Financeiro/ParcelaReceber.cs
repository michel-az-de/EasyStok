...
/// <summary>
/// Parcela de uma <see cref="ContaReceber"/>. Suporta opcionalmente Pix QR Code
/// gerado via Efi (txid + copia-cola + qrcode base64). Reconciliacao por webhook
/// ou job horario consulta o gateway pelo <see cref="EfiTxid"/>.
/// </summary>
public class ParcelaReceber
{
    ...
    public static ParcelaReceber Criar(
        Guid contaReceberId,
        Guid empresaId,
        int numero,
        decimal valor,
    ...
    public void RegistrarPagamento(PagamentoParcela pagamento)
    {
        ...
        if (pagamento.Lado != TipoLadoFinanceiro.Receber)
            throw new RegraDeDominioVioladaException("Pagamento e do lado contrario (esperado Receber).");
    ...
    public void AssociarPix(string txid, string copiaCola, string qrCodeBase64, DateTime expiraEm)
    {
        if (Status == StatusParcela.Paga || Status == StatusParcela.Cancelada)
            throw new RegraDeDominioVioladaException("Nao e possivel associar Pix em parcela paga ou cancelada.");
        if (string.IsNullOrWhiteSpace(txid))
            throw new RegraDeDominioVioladaException("Txid do Pix nao pode ser vazio.");
