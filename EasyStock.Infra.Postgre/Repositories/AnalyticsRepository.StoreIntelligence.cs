...
{
    /// <summary>
    /// Partial: Store Intelligence (comparação de lojas, health score, indicadores
    /// de ação, top produtos por loja). Extraido do god-AnalyticsRepository (F12).
    ///
    /// Usa os fields do facade (dbContext, cache, _dashboard) via partial — não
    /// duplica state. Cache helpers GetCachedAsync/SetCachedAsync vivem no arquivo
    /// principal.
    /// </summary>
        ...
        public async Task<IReadOnlyList<LojaComparacao>> GetComparacaoLojasAsync(Guid empresaId, int periodoDias = 30)
        {
            ...
            var stockByLoja = await dbContext.ItensEstoque.AsNoTracking()
                ...
                .Select(g => new
                {
                    ...
                    ValorEstoque = g.Sum(i => ((decimal?)i.PrecoVendaSugerido ?? (decimal)i.CustoUnitario * Domain.Defaults.OperacionalDefaults.FallbackMargemPrecoSugerido) * (int)i.QuantidadeAtual),
